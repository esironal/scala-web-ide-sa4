...
object Compilable {
   ...
   */
  implicit def toXml(link: Compilable): Node =
    <link>{Xml.toXml(link)}</link>
   ...
   */
  implicit def toJson(link: Compilable): JValue =
    Extraction.decompose(link)
   ...
   * a Compilable can be returned easily from a JSON call
   */
  implicit def toJson(links: Seq[Compilable]): JValue =
    Extraction.decompose(links)
  ...
  private def data =
"""[
  {"id": "1234", "path": "/Users/schapfr/Desktop/json-obj.json",
  "options": ["-target", "-d", "-verbose"]
  },
  {"id": "1235", "path": "/Users/schapfr/Desktop/json-obj.json",
  "options": ["-target", "-d"]
  },
  {"id": "1236", "path": "/Users/schapfr/Desktop/json-obj.json",
  "options": ["-d", "-verbose"]
  },
  {"id": "1237", "path": "/Users/schapfr/Desktop/json-obj.json",
  "options": ["-target","-verbose"]
  },
]
"""
  ...
  def search(str: String): List[Compilable] = {
    ...
    links.filter(i =>
      i.id.toLowerCase.indexOf(strLC) >= 0 ||
                 i.path.toLowerCase.indexOf(strLC) >= 0)
  }
  ...
  private def updateListeners(link: Compilable): Compilable = {
    synchronized {
      listeners.foreach(f => 
        Schedule.schedule(() => f(link), 0 seconds))
